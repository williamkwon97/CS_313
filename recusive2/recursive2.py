...
def groupSumClump(start, nums, target):
    if (start>=len(nums)):
        if(target==0):
            ...
            return False
    ints=start +1
    while (ints<len(nums) and nums[start]==nums[ints] ):
        ints+=1
    if (groupSumClump(ints,nums,target-((ints-start)*nums[start]))):
        return True
    return groupSumClump(ints,nums,target)
...
def split53Helper(start,nums,g5,g3):
   if(start>=len(nums)):
       return g5==g3
   if(nums[start]%5 ==0):
       return split53Helper(start+1,nums,g5+nums[start],g3)
   if(nums[start]%3==0):
       return split53Helper(start+1,nums,g5,g3+nums[start])
   if(split53Helper(start+1,nums,g5+nums[start],g3)):
       return True
   if(split53Helper(start+1,nums,g5,g3+nums[start])):
       return True
   return False
